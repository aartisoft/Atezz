...
class Api_gigs_model extends CI_Model
{
    ...
    public function popular_gigs_image(){
        ...
        $this->db->select('SG.id,replace(SG.title,"-", " ") as title , GI.image_path AS image');
    ...
    public function popular_gigs_list($id = ''){
     ...
     $query_string = "SELECT SG.id,SG.user_id,SG.delivering_time as delivering_days,SG.currency_type,CASE SG.currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign, replace(SG.title,'-', ' ') as title ,SG.gig_price,SG.total_views,M.fullname,C.country,S.state_name, GI.gig_image_medium AS image,(SELECT count(id) FROM `feedback` WHERE `gig_id` = SG.id and to_user_id = SG.user_id) AS gig_usercount,
        ...
        $query_string .= " ORDER BY total_views DESC LIMIT 0,5"; 
    ...
    public function recent_gigs_list($id = ''){
        ...
        $query_string .= " ORDER BY SG.id DESC LIMIT 0,5"; 
    ...
    public function categoriesandgigs($category_id,$sub_categoryid,$user_id,$services){
            ...
            if(empty($category_id) && empty($sub_categoryid) &&  empty($user_id) && ($services=='all')){
                 ...
                 $query_string .= " WHERE SG.status = 0  ORDER BY SG.id ASC"; 
               ...
               // Buyser  Gigs List   
                 ...
                 $query_string .= " WHERE SG.status = 0 AND SG.user_id != $user_id  ORDER BY SG.id ASC"; 
...

               {
                    ...
                    if(empty($sub_categoryid)){
                        ...
                        if(!empty($sub_categoryid)){
                         ...
                         $query_string .= " WHERE SG.status = 0 AND category_id = $sub_categoryid AND CA.parent = $category_id";
...

                {
                 ...
                 // No sub category, with and with out login 
                     ...
                     $query_string .= " WHERE SG.status = 0 AND category_id = $category_id";
     ...
     public function my_gigs_list($userid){
        ...
        if($userid!=0){
          ...
          $query_string = "SELECT SG.id,SG.user_id,SG.delivering_time as delivering_days,SG.currency_type,CASE SG.currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign, replace(SG.title,'-', ' ') as title ,SG.gig_price,SG.total_views,M.fullname,C.country,S.state_name, GI.gig_image_medium AS image,(SELECT count(id) FROM `feedback` WHERE `gig_id` = SG.id and to_user_id = SG.user_id) AS gig_usercount,
                ...
                WHERE SG.status = 0 AND SG.user_id = $userid  ORDER BY SG.id ASC"; 
  ...
  if($gigid!=0){
          ...
          $query_string = "SELECT SG.id,SG.user_id,SG.delivering_time as delivering_days,SG.currency_type,CASE SG.currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign, replace(SG.title,'-', ' ') as title ,SG.gig_price,SG.category_id,SG.gig_details,SG.requirements,SG.super_fast_charges,SG.super_fast_delivery_desc,SG.super_fast_delivery_date as super_fast_days ,SG.total_views,C.country,S.state_name, GI.gig_image_medium AS image,(SELECT count(id) FROM `feedback` WHERE `gig_id` = SG.id and to_user_id = SG.user_id) AS gig_usercount,

                (SELECT COALESCE(CAST(AVG(rating) AS DECIMAL(10,1)),0) as rating FROM `feedback` WHERE `gig_id` = SG.id and to_user_id = SG.user_id) AS gig_rating , M.email,M.username,M.fullname,M.user_timezone,M.verified,M.status,M.city,M.address,M.zipcode,M.lang_speaks,M.country AS country_id,M.state,M.profession,M.contact,M.description,M.user_thumb_image,M.user_profile_image ,'0' as favourite  FROM sell_gigs AS SG  LEFT JOIN gigs_image AS GI  ON GI.gig_id = SG.id";
        ...
        WHERE SG.status = 0 AND SG.id = $gigid   ORDER BY SG.id ASC"; 
        ...
        if ($query->num_rows() > 0) {
            ...
            $similar_query_string = "SELECT SG.id,SG.user_id,SG.delivering_time as delivering_days,SG.currency_type,CASE SG.currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign, replace(SG.title,'-', ' ') as title ,SG.gig_price,SG.category_id,GI.gig_image_medium AS image,(SELECT count(id) FROM `feedback` WHERE `gig_id` = SG.id and to_user_id = SG.user_id) AS gig_usercount, (SELECT COALESCE(CAST(AVG(rating) AS DECIMAL(10,1)),0) as rating FROM `feedback` 

            WHERE `gig_id` = SG.id and to_user_id = SG.user_id) AS gig_rating, C.country,S.state_name,M.fullname,'0' as favourite FROM sell_gigs AS SG
        ...
        WHERE SG.status = 0 AND SG.category_id= $category_id AND SG.id != $category_gig_id ";

        if(!empty($userid)){ $similar_query_string .= " AND SG.user_id != $userid";       }
     ...
     public function seller_buyer_review($userid,$gid,$limit){
        ...
        $this->db->select('gig_id,comment,F.rating,time_zone,F.created_date,to_user_id,from_user_id,M.fullname AS sellername,M1.fullname AS buyername');
...

{
 ...
 public function create_gigs($data){
  ...
  $gigs_details['work_option'] = $data['work_option'];
    ...
    $time_zone = (empty($data['work_option'])?'Asia/Kolkata':$data['work_option']);
  ...
  if($this->db->insert('sell_gigs',$gigs_details)){
      ...
      /* No image Start */
       ...
       $data_image['image_path'] = 'uploads/gig_images/noimage.jpg';

       $data_image['gig_image_thumb'] = 'uploads/gig_images/noimage.jpg';

       $data_image['gig_image_tile'] = 'uploads/gig_images/noimage.jpg';

       $data_image['gig_image_medium'] = 'uploads/gig_images/noimage.jpg';
 ...
 public function update_gigs($data){
  ...
  if($this->db->update('sell_gigs',$gigs_details)){
      ...
      /* No image Start */
       ...
       // $data_image['image_path'] = 'noimage_l.jpg';

       // $data_image['gig_image_thumb'] = 'noimage_t.jpg';

       // $data_image['gig_image_tile'] = 'noimage.jpg';

       // $data_image['gig_image_medium'] = 'noimage_m.jpg';
  ...
  // end here  
   ...
   public function edit_details($where){
    ...
    $this->db->select('user_id,replace(title,"-", " ") as title'.",SG.currency_type,CASE SG.currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign,SG.id,gig_price,delivering_time,category_id,C.parent as sub_categoryid,gig_tags,gig_details,super_fast_delivery,super_fast_delivery_desc,super_fast_delivery_date,super_fast_charges,requirements,work_option");
   ...
   public function favourites_gigs($user_id){
     ...
     $query_common_query = "SELECT SG.id,SG.user_id,SG.delivering_time as delivering_days,SG.category_id,SG.currency_type,CASE SG.currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign, replace(SG.title,'-', ' ') as title ,SG.gig_price,SG.total_views,M.fullname,C.country,S.state_name, GI.gig_image_medium AS image,(SELECT count(id) FROM `feedback` WHERE `gig_id` = SG.id and to_user_id = SG.user_id) AS gig_usercount,
                ...
                LEFT JOIN states AS S  ON S.state_id = M.state WHERE SG.status = 0 AND SG.id in (SELECT gig_id FROM favourites WHERE user_id = $user_id)";
    ...
    public function last_visited_gigs($user_id){
     ...
     $query_common_query = "SELECT SG.id,SG.user_id,SG.delivering_time as delivering_days,SG.category_id,SG.currency_type,CASE SG.currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign, replace(SG.title,'-', ' ') as title ,SG.gig_price,SG.total_views,M.fullname,C.country,S.state_name, IF(GI.gig_image_medium = 'noimage.jpg','',GI.gig_image_medium)  AS image,(SELECT count(id) FROM `feedback` WHERE `gig_id` = SG.id and to_user_id = SG.user_id) AS gig_usercount,
                ...
                WHERE SG.status = 0 AND LV.user_id = $user_id ORDER BY LV.created_date DESC";
...
public function get_user_orders_total($user_id){
      ...
      WHERE py.`USERID` = $user_id ORDER BY py.`created_at` DESC  ");
       ...
       if(!empty($records)){
          ...
          foreach ($records as $key => $value) {
           ...
           if($buyer_status ==0 && $status !=6 && $status_msg != 'Completed Accept'){
  ...
  public function get_selluser_details($user_id){
    ...
    $query = $this->db->query("SELECT py.USERID as user_id,sg.user_id as from_user_id,py.id as order_id, DATE_FORMAT(py.created_at,'%d %M %Y %H:%i') as created_date,py.currency_type,CASE py.currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign,replace(sg.title,'-', ' ') as title, DATE_FORMAT(DATE_ADD(py.delivery_date, INTERVAL sg.delivering_time DAY) ,'%d %b %Y') as delivery,m.fullname as buyer_name ,py.item_amount as amount,py.pay_status ,py.delivery_date, py.gigs_id, py.extra_gig_ref, py.time_zone, py.seller_id,  py.payment_status, py.decline_accept, py.seller_status, py.cancel_accept, py.cancel_reason, py.buyer_status, py.status,py.created_at,m.username,(SELECT gigs_image.`gig_image_thumb` FROM `gigs_image` WHERE gigs_image.gig_id =  py.gigs_id LIMIT 0 , 1 ) AS gig_image_thumb  FROM `payments` as py
...

  {
    ...
    $query = $this->db->query("SELECT sum(py.item_amount) as amount FROM payments as py 
                               ...
                               WHERE py.seller_id = $user_id AND seller_status = 6 "); //  AND py.payment_status =1
  ...
  public function seller_reviews($user_id){



    $query = $this->db->query("SELECT from_user_id,to_user_id,gig_id,comment,F.rating,F.time_zone,F.created_date,currency_type,CASE currency_type when 'USD'  then '$' when 'EUR'  then '€' when 'GBP'  then '£' END as currency_sign,replace(SG.title,'-',' ') as title, M.fullname as buyer_name  FROM  feedback F LEFT JOIN sell_gigs As SG  ON SG.id = F.gig_id  LEFT JOIN members M ON M.USERID = F.from_user_id where to_user_id = $user_id");
  ...
  public function seefeedback($data){
...

    {
...

      {
        ...
        $feedback_array[0]['fb_user_name'] = $result["username"];

        $feedback_array[0]['fb_user_url'] = base_url().'user-profile/'.$result["username"];

        $feedback_array[0]['fb_user_img'] = $user_img;

        $feedback_array[0]['fb_user_time'] = $tme;

        $feedback_array[0]['fb_user_comment'] = $comment;

        $feedback_array[0]['fb_user_rating'] = $rating;
        ...
        if($result_array){
          ...
          $feedback_array[1]['fb_user_name'] = $result_array["username"];

          $feedback_array[1]['fb_user_url'] = base_url().'user-profile/'.$result_array["username"];

          $feedback_array[1]['fb_user_img'] = $user_img1;

          $feedback_array[1]['fb_user_time'] = $tme1;

          $feedback_array[1]['fb_user_comment'] = $comment1;

          $feedback_array[1]['fb_user_rating'] = $rating1;
...
public function save_feedback($where_data)
  ...
  public function chart_user_details($id) { 
    ...
    public function get_chat_details($f_user,$t_user,$page){
     ...
     //     $query_string .= "LIMIT 0 ,10";  
      ...
      return array('total_records'=>ceil($total_records/10),'chat_details'=>$data);
 ...
 public function search_data($input){
    ...
    if(!empty($title) || !empty($category_id) || !empty($state) || !empty($country)){
      ...
      if(!empty($title) && !empty($category_id) && !empty($state) && !empty($country)){
        ...
        $query_string .= " WHERE title like '%".$title."%' AND category_id = $category_id AND M.state = $state AND M.country = $country";   

      }

      if(empty($title) && !empty($category_id) && !empty($state) && !empty($country)){

        $query_string .= " WHERE  category_id = $category_id AND M.state = $state AND M.country = $country";   
      ...
      if(!empty($title) && !empty($category_id) && !empty($state) && empty($country)){
        ...
        $query_string .= " WHERE title like '%".$title."%' AND category_id = $category_id AND M.state = $state ";   

      }

      if(!empty($title) && !empty($category_id) && !empty($country) && empty($state)){

        $query_string .= " WHERE title like '%".$title."%' AND category_id = $category_id AND M.country = $country";   
      ...
      if(empty($title) && !empty($category_id) && !empty($state) && empty($country)){
        ...
        $query_string .= " WHERE category_id = $category_id AND M.state = $state ";   

      }

      if(empty($title) && !empty($category_id) && empty($state) && !empty($country)){

        $query_string .= " WHERE  category_id = $category_id  AND M.country = $country";   

      }

      if(!empty($title) && !empty($category_id) && empty($country) && empty($state)){

        $query_string .= " WHERE title like '%".$title."%' AND category_id = $category_id";   
  ...
  public function GetAllFooter(){
      ...
      $this->db->select('footer_menu.title');

      $this->db->select('footer_submenu.footer_submenu,footer_submenu.page_desc');
